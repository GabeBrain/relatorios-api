...
function row(periodKey: string, sold: number): ClosureRow {
  const period = `${periodKey}-01`;
  const year = periodKey.slice(0, 4);
  return {
    building_id: '1', building_name: 'Empreendimento', building_type: 'Vertical', standard: 'Médio', city: 'Cidade',
...
describe('computeResumoEmail', () => {
  it('usa o último período selecionado e limita os acumulados de janeiro até ele', () => {
    const dimensionRows = [row('2025-01', 50), row('2025-07', 80), row('2026-01', 60), row('2026-06', 100), row('2026-07', 120)];
