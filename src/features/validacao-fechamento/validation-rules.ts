...
export interface Divergence {
  building_id: string; building_name: string; typology_id: string; private_area: number | null;
...
export function validateBuildings(buildings: ValidationBuilding[]): Divergence[] {
  ...
  const out: Divergence[] = []; const emit = (r: Row, field: string, error: string, value: string, rule: string) => out.push({ building_id: r.b.building_id, building_name: r.b.name, typology_id: r.typologyId, private_area: r.h.private_area, field, error, value, rule });
