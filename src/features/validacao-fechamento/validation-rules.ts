...
export function validateBuildings(buildings: ValidationBuilding[]): Divergence[] {
  ...
  const emit = (r: Row, field: string, error: string, value: string, rule: string) => { const cityComparison = /cidade|média do grupo|Vertical com preço/i.test(error) && !value.includes('Média cidade:'); out.push({ building_id: r.b.building_id, typology_id: r.typologyId, building_name: r.b.name, private_area: r.h.private_area, city: r.b.city, status: r.h.building_status, period: r.h.period, field, error, value: cityComparison ? `${value} | Média cidade: ${fmt(r.cityPriceM2)}` : value, rule, group: enterpriseErrors.has(error) ? 'Empreendimento: building_id + período' : 'Cidade: cidade + tipo do empreendimento + tipo de tipologia + padrão + dormitórios + período' }); };
