...
export const ERROR_CATALOG: Record<ErrorType, ErrorMeta> = {
  ...
  CROSS_TABLE_MISMATCH: { label: 'Inconsistência entre tabelas', description: 'Valores repetidos em slides diferentes devem ser idênticos (rendas, totais, população).', mode: 'PLENO', viz: 'sidebyside', motor: 'DET' },
  ...
  TEMPORAL_WINDOW: { label: 'Janela temporal', description: 'Projeções cobrem a janela canônica de 6 anos; slides irmãos usam a mesma base.', mode: 'PLENO', viz: 'sidebyside', motor: 'DET' },
  PROJECTION_FORMULA: { label: 'Fórmula de projeção', description: 'A projeção deve seguir a fórmula oficial (depende da taxa do Brasil).', mode: 'MOCK', viz: 'table', motor: 'DET' },
  ...
  SOURCE_MISSING: { label: 'Fonte ausente', description: 'Slide com tabela/gráfico deve indicar FONTE e ELABORAÇÃO em texto.', mode: 'PLENO', viz: 'overlay', motor: 'DET' },
  ...
  ATA_COVERAGE: { label: 'Cobertura da ata', description: 'Tudo que o cliente pediu na ata deve existir no estudo.', mode: 'MOCK', viz: 'checklist', motor: 'DET+IA' },
