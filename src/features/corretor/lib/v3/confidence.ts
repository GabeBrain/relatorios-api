...
import { ERROR_CATALOG, isCommunication, type ErrorType } from '../error-catalog';
...
export function confidenceOf(finding: Pick<Finding, 'type' | 'detail' | 'escalated' | 'evidenceSha1' | 'confidence'>, origem = 'DET'): Confidence {
  ...
  // A regra que sabe mais que o tipo manda (tabela paginada, exclusão declarada).
  if (finding.confidence) return finding.confidence;
  // Recado da revisão não é erro; nunca bloqueia a entrega pela confiança.
  if (isCommunication(type)) return 3;
