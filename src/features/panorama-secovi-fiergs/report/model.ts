...
import type { LaunchRecord, MarketCohortRow, MethodStatus, PanoramaReportModel, PanoramaScope, Quarter, ReportMarketBlock, Segment } from '../types';
...
function cohortBlock(scope: PanoramaScope, rows: MarketCohortRow[]): ReportMarketBlock {
  ...
  for (const row of rows) { const group = groups.get(row.releaseYear) ?? { vertical: 0, horizontal: 0 }; group[row.segment.toLowerCase() as 'vertical' | 'horizontal'] += row.stock; groups.set(row.releaseYear, group); }
  const status = rows.length ? 'ready' : 'unavailable';
  return { series: canonical(scope).map((quarter) => ({ quarter, vertical: 0, horizontal: 0, total: 0, methodStatus: 'assumed', dataStatus: status, source: 'building-with-history' })), byGroup: [...groups].sort(([a], [b]) => a.localeCompare(b)).map(([label, value]) => ({ label, ...value, total: value.vertical + value.horizontal })), unit: 'count', methodStatus: 'assumed', dataStatus: status, source: 'building-with-history / typologies_history', formula: 'Último snapshot até o fechamento, agrupado pelo ano de release_date.' };
}
export function buildPanoramaReportModel(scope: PanoramaScope, records: LaunchRecord[], sources: { sales: SourceResult; stock: SourceResult; ivv: SourceResult; ticket: SourceResult; meter: SourceResult }, cohorts: MarketCohortRow[] = []): PanoramaReportModel {
  ...
  return { scope, generatedAt: new Date().toISOString(), launches,
    ...
    market: { cohorts: cohortBlock(scope, cohorts) },
