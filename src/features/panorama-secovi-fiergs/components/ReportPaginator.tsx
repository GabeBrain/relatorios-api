...
function Corporate({ page, report }: { page: number; report: PanoramaReportModel }) {
  ...
  if (report.scope.entity === 'fiergs-rs') {
    ...
    if (page === 8) return <div className="panorama-fiergs-about"><header><p>PANORAMA DO MERCADO IMOBILIÁRIO</p><h2>Sobre o estudo FIERGS</h2><i/></header><main><section><strong>10</strong><span>municípios analisados</span><small>Recorte metropolitano sem Porto Alegre no universo numérico.</small></section><section><strong>2</strong><span>segmentos residenciais</span><small>Mercado vertical e quatro produtos horizontais identificados separadamente.</small></section><section><strong>75</strong><span>lâminas automatizadas</span><small>Séries históricas, fechamento, preços, oferta, vendas e localização.</small></section></main><footer>Dados GeoBrain · regras e rastreabilidade preservadas no cubo granular e na auditoria CSV.</footer></div>;
  }
