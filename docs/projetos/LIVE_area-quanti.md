...
- **APIs:** o acesso `APIs/Explorer` foi ocultado da sidebar e da busca global, mantendo a rota `/apis/explorer` e toda a funcionalidade disponível por link direto.
...
- **Tema:** o arquivo exportado usa fundo branco e contraste claro, independentemente do modo escuro do dashboard.
...
- **Correção:** legendas nativas dos gráficos e legendas personalizadas da Análise Cruzada também são incorporadas ao SVG.
...
- **Ordenação A-Z:** gráficos categóricos ganharam botão de ordem alfabética/numérica com ciclo `A-Z` → `Z-A` → padrão; prefixos internos como `1.`, `2.` e `0.` seguem sendo usados para ordenar faixa etária, geração, renda e tempo para compra, mas deixam de aparecer para o usuário nos rótulos dos gráficos e heatmaps.
- **Base colunar:** o carregador da Base Unificada 2020 passou a reconciliar aliases ao expandir JSON colunar, preservando a regra `Cidade (coleta)` ← `Cidade`, `Cidade (empreendimento)` ← `Cidade_Original` e `Estado` ← `Estado_Original`, além de mapear `Latitude`/`Longitude`; isso evita KPIs zerados quando a planilha/JSON muda capitalização, underscores ou rótulos de coluna.
- **Análise Cruzada Universal:** os seletores de perguntas/campos agora respeitam a ordem original das colunas no arquivo, em vez de reorganizar alfabeticamente.
- **Performance da Análise Cruzada:** a abertura do pivot agora inicia sem perguntas selecionadas e só calcula após escolha do usuário; matrizes acima de 1.500 células exibem confirmação antes de renderizar.
